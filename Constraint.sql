...
# 제약조건을 통해서 DBMS의 무결성을 보장해줌

# NOT NULL
# NOT NULL 제약조건이 지정되어있는 컬럼에는 삽입 및 수정 작업에서 필수로 데이터를 입력해야 함
...
INSERT INTO not_null_table (column1, column2, not_null_column) VALUES (1, 2, 3);

UPDATE not_null_table SET not_null_column = null;

# UNIQUE
# UNIQUE 제약조건이 지정되어있는 컬럼에는 각 레코드에서 해당하는 컬럼 값을 중복하여 삽입 혹은 수정 할 수 없음
...
# Candidate Key (후보키)
# 각 레코드를 구분할 수 있는 키로 중복을 허용하지 않고 필수로 값을 가지고 있어야함
...
# PRIMARY KEY (기본키)
# PRIMARY KEY 제약조건이 지정되어 있는 컬럼은 해당 레코드를 유일하게 식별하는 컬럼이 됨
# PRIMARY KEY는 후보키 중 선택된 하나의 컬럼이 PRIMARY KEY가 됨
# PRIMARY KEY 제약조건이 지정되어 있는 컬럼은 NOT NULL, UNIQUE 제약조건을 포함하고 있음
...
CREATE TABLE primary_table_2(
	primary_column INT,
    clumn1 INT,
    CONSTRAINT primary_table_pk PRIMARY KEY (primary_column)
);

# CHECK
# CHECK 제약조건이 지정되어 있는 컬럼에는 삽입 및 수정 작업시에 데이터의 유효성을 검증함
...
# DEFAULT
# DEFAULT 제약조건이 지정되어 있는 컬럼에는 삽입 작업시에 값을 지정하지 않으면 (null이 지정될 때) 삽입될 기본값을 명시함
...
INSERT INTO default_table VALUES (null, 1);
...
# 1. 만약 테이블을 삭제 하려고 할 때 만약 해당 테이블을 참조하고있는 테이블이 있다면 삭제할 수 없음
# 2. 특정 컬럼에 참조제약조건을 지정하려 한다면 참조하는 컬럼의 데이터타입과 일치해야함
# 3. 피참조 테이블의 수정 및 삭제 작업시에 해당 컬럼의 값을 참조하고 있는 테이블이 있다면 수정 및 삭제 작업이 실패함
...
# FREIGN KEY 제약조건이 지정되어있는 컬럼의 값은 참조하고 있는 테이블의 컬럼에 있는 데이터만 지정할 수 있음
...
# CASCADE : 참조하고 있는 데이터가 수정 또는 삭제가 됐을 때 참조하는 레코드에서도 삭제 및 수정이 이루어짐
# SET NULL : 참조하고 있는 데이터가 수정 또는 삭제가 됐을 때 참조하는 레코드의 해당 컬럼 값을 null로 지정
...
UPDATE super_table SET primary_column = 10 WHERE primary_column = 3;
