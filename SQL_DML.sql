...
# WHERE
# DML 중 검색, 수정, 삭제에 대해서 특정한 조건에 부합하는 레코드에 대해서만 작업이 이루어 지도록 하는 연산자

# WHERE 조건을 잘 적는것 만으로도 대부분의 작업을 수월하게 할 수 있음
# WHERE 조건에는 비교연산자 / 논리연산자가 들어옴
...
# NOT BETWEEN min AND max : 좌항이 min 보다 작으면서 max 보다 크면 true, 아니면 false
...
UPDATE namgu SET `면적_부피_길이(제곱미터)` = null WHERE `면적_부피_길이(제곱미터)` = '' OR `면적_부피_길이(제곱미터)` = '㎡';
UPDATE namgu SET 위치_소재지 = '변경 소재지' WHERE 해체제거업자사업체명 = '우성건설' AND 순번 = 3;

# 해체제거업자사업체명 '우성건설'인 데이터의 '건물명_설비명', '위치_소재지'
SELECT 건물명_설비명, 위치_소재지 FROM namgu WHERE 해체제거업자사업체명 = '우성건설';

# 해체제거업자사업체명 '우성건설'이 아닌  데이터의 모든 컬럼을 검색
...
# 해체시작년월일 이 2022-05-01 ~ 2022-06-30 사이인 데이터의 모든 컬럼을 검색
SELECT * FROM namgu WHERE 해체시작년월일 BETWEEN '2022-05-01' AND '2022-06-30';

# 해체시작년월일 이 2022-05-01 ~ 2022-06-30 사이가 아닌 데이터의 모든 컬럼을 검색
SELECT * FROM namgu WHERE 해체시작년월일 NOT BETWEEN '2022-05-01' AND '2022-06-30';

# 논리연산자
# AND, && : 좌항과 우항이 모두 true일 때 true를 반환, 아니면 false를 반환
# OR, || : 좌항과 우항 중 하나라도 true 일 때 true를 반환, 아니면 false를 반환
# XOR : 좌항과 우항이 서로 다르면 true를 반환, 같으면 false를 반환
# NOT, ! : 우항이 true면 false를 반환, false면 true를 반환

# 해체제거업자사업체명이 '성수개발' 이거나 '우성건설' 이거나 '고은산업'인 데이터의 모든 컬럼을 검색
SELECT * FROM namgu WHERE 해체제거업자사업체명 = '성수개발'
OR 해체제거업자사업체명 = '우성건설'
OR 해체제거업자사업체명 = '고은산업';
...
# 2. '%용호' :  앞에 어떠한 문자가 오든 '용호'라는 단어로 끝나면 true
# 3. '용호%' :  뒤에 어떠한 문자가 오든 '용호'라는 단어로 사직하면 true
# 4. '_용호_' : 앞뒤에 한 문자가 오고 '용호'라는 단어를 포함하고 길이가 4면 true
# 5. '_용호' : 앞에 한 문자가 오고 '용호'라는 단어로 끝나고 길이가 3이면 true
# 5. '용호_' : 뒤에 한 문자가 오고 '용호'라는 단어로 시작하고 길이가 3이면 true

# 위치_소재지에 '용호로'가 포함되어 있는 데이터의 전체 컬럼을 검색
SELECT * FROM namgu WHERE 위치_소재지 LIKE '%용호로%';

# 해체제거업자사업체명이 '(주)'로 사직하는 데이터의 전체 컬럼을 검색
...
# ASC는 생략 가능하고 만약 여러 개의 컬럼을 ORDER BY문에 지정했다면 먼저 작성한 컬럼 순으로 정렬
