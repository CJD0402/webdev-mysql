# 회원가입
INSERT INTO user VALUES  ('email@email.com', 'P!ssw0rd', 'nickname', '010-1111-1111', '부산광역시 부산진구', '에이원프라자 4층' , null);
...
# 게시글 작성
INSERT INTO board (writer_email, title, content, image) VALUES ('email@email.com', '게시글 제목', '게시글 내용', null);
INSERT INTO board (writer_email, title, content, image) VALUES ('email@email.com', '게시글 제목2', '게시글 내용2', null);
INSERT INTO board (writer_email, title, content, image) VALUES ('email@email.com', '게시글 제목3', '게시글 내용3', null);
INSERT INTO board (writer_email, title, content, image) VALUES ('email2@email.com', '게시글 제목4', '게시글 내용4', null);

# TOP3 게시물
# 게시물 번호 / 게시물 제목 / 게시물 내용 / 게시물 이미지 / 조회수 / 작성일 / 작성자 프로필 사진 / 작성자 닉네임 [/ 댓글 수 / 좋아요 수]

SELECT * FROM board;

# Alias
# 컬럼이나 테이블에 별칭을 지정하고 할 때 컬럼이나 테이블 뒤에 'AS' 를 붙여 별칭을 부여할 수 있음
SELECT B.board_number AS '게시물 번호', writer_email AS '작성자 이메일' FROM board AS B;
...
# 서브쿼리를 이용하면 복잡한 검색조건을 사용 할 수 있음

# WHERE 절에서 서브쿼리

# 예시) nickname이 'nickname'인 유저가 작성한 게시물의 전체 컬럼을 검색
SELECT * FROM board
WHERE writer_email
IN (SELECT email FROM user WHERE nickname = 'nickname');
...
SELECT board_number, (SELECT nickname FROM user WHERE email = 'email@email.com') AS nickname
FROM board
WHERE writer_email = 'email@email.com';

# FROM 절에서 서브쿼리
SELECT * FROM(SELECT * FROM user WHERE adress = '부산광역시 부산진구') AS T1;

SELECT * FROM board
INNER JOIN (SELECT * FROM user WHERE adress = '부산광역시 부산진구') AS T1
...
# MAX, MIN
# 특정 컬럼의 최대, 최소 값을 구함
SELECT max(view_count), MIN(view_count) FROM board;

USE sample_database;

# GROUP BY
# SELECT 문에서 반환되는 행들을 하나 이상의 컬럼을 기준으로 그룹화 할 때 사용
SELECT * FROM namgu;
...
# HAVING
# GROUP BY 결과에 조건을 걸어서 출력(필터링)을 하기 위한 구문
# WHERE 절과 비슷한 사용방법을 가지고 있음
